  ...
  return (
    <div className="shell py-12 md:py-16">
      ...
      <div className="mt-12 grid gap-px border hairline bg-[#d5d1c7] lg:grid-cols-2">
        {benchmarks.map((benchmark, index) => <article key={benchmark.id} className="bg-[var(--paper)] p-6 md:p-8"><div className="flex items-center justify-between"><span className="grid h-10 w-10 place-items-center bg-black text-xs font-black text-white">0{index + 1}</span>{index === 0 ? <Gauge /> : <GitBranch />}</div><h2 className="mt-10 text-3xl font-black tracking-[-.04em]">{benchmark.name}</h2><p className="mt-4 text-sm leading-6 text-[#5f5b54]">{benchmark.scope}</p><div className="mt-8"><div className="eyebrow">LIMITATIONS</div><ul className="mt-3 space-y-3">{benchmark.limitations.map(item => <li key={item} className="flex gap-2 text-xs leading-5"><span className="mt-2 h-1.5 w-1.5 shrink-0 bg-black" />{item}</li>)}</ul></div><div className="mt-8 border border-[#dfb646] bg-[#fff5d6] p-4 text-xs leading-5"><strong>授权边界：</strong>{benchmark.redistribution}</div><div className="mt-6 flex flex-wrap gap-2"><a href={benchmark.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 border border-black bg-black px-4 py-3 text-xs font-black !text-white visited:!text-white hover:bg-[var(--blue)] hover:!text-white">打开评测 <ArrowUpRight size={14} /></a><a href={benchmark.methodologyUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 border border-black px-4 py-3 text-xs font-black visited:text-black hover:bg-black hover:!text-white">查看方法 <ArrowUpRight size={14} /></a></div></article>)}
      </div>
