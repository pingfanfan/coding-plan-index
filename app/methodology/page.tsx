...
const rules = [
  ...
  [
    RefreshCw,
    "自动扫描与人工核验分开",
    "机器人每天检测页面是否失效或关键正文是否变化。检测到变化会创建审核 Issue，但价格只有人工确认后才更新核验日期。",
  ],
];
    ...
    <div className="shell py-12 md:py-16">
      ...
      <section className="mt-16 grid gap-8 border-t border-black pt-10 lg:grid-cols-[300px_1fr]">
        ...
        <div className="space-y-5 text-sm leading-6 text-[#5f5b54]">
          ...
          <p>
            Logo 外五段不完整光环表示 1–5
            档“可用量强度”，亮起一段对应一档；低置信度估计使用虚线。它优先根据官网精确额度、相对倍率、刷新窗口和超额方式分档；官网没有绝对数字时只按厂商自己声明的相对档位估计。这个档位不把
            Kiro credit、AI Credit、请求或 token 相互换算。
          </p>
          <p>
            虚线只连接价格更低、智能估计更高的二维参考点；用量没有参与虚线，因此不声称是完整的帕累托最优。免费套餐、纯按量
            API、自定义报价和缺少可追溯估计的套餐不进入坐标，但仍保留在目录和详情页。
          </p>
        </div>
      </section>
      <section className="mt-16 grid gap-8 border-t border-black pt-10 lg:grid-cols-[300px_1fr]">
