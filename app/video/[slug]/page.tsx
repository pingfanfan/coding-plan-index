...
function formatRate(rate: VideoRate) {
  ...
  const unit = { credits_per_second: "credits / 秒", credits_per_clip: "credits / 次", usd_per_second: "USD / 秒", cny_per_second: "CNY / 秒", units_per_clip: "units / 次", undisclosed: "" }[rate.billingUnit];
  const prefix = rate.billingUnit === "usd_per_second" ? "$" : rate.billingUnit === "cny_per_second" ? "¥" : "";
  return `${prefix}${rate.amount} ${unit}`;
}
function formatFiveSecondCost(rate: VideoRate) {
  if (!rate.fiveSecondCost) return "—";
  const prefix = rate.fiveSecondCost.currency === "USD" ? "$" : rate.fiveSecondCost.currency === "CNY" ? "¥" : `${rate.fiveSecondCost.currency} `;
  return `${prefix}${rate.fiveSecondCost.amount.toFixed(2)}`;
}
...
export default async function VideoProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="shell py-10 md:py-14">
    ...
    <header className="mt-10 grid gap-8 border-b border-black pb-10 lg:grid-cols-[1fr_320px]">
      ...
      <div className="grid grid-cols-2 gap-px bg-[#d5d1c7] text-xs"><div className="bg-white/50 p-4"><span className="text-[#6f6b63]">最高分辨率</span><strong className="mt-2 block">{product.maxResolution}</strong></div><div className="bg-white/50 p-4"><span className="text-[#6f6b63]">原生音频</span><strong className="mt-2 block">{product.nativeAudio === null ? "因模型而异" : product.nativeAudio ? "支持" : "未支持"}</strong></div><div className="bg-white/50 p-4"><span className="text-[#6f6b63]">API</span><strong className="mt-2 block">{product.apiAvailable ? "有" : "未提供"}</strong></div><div className="bg-white/50 p-4"><span className="text-[#6f6b63]">最后核验</span><strong className="mt-2 block">{product.verifiedAt}</strong></div></div>
    </header>

    <section className="py-12"><div className="eyebrow">SUBSCRIPTION PLANS</div><h2 className="mt-2 text-3xl font-black tracking-[-.04em]">当前套餐</h2>{product.plans.length ? <div className="mt-7 overflow-x-auto border hairline"><table className="w-full min-w-[980px] border-collapse text-left text-xs"><thead className="bg-black text-white"><tr>{["套餐", "月价", "包含额度", "刷新/结转", "视频权益", "用尽后", "状态"].map((x) => <th className="p-4" key={x}>{x}</th>)}</tr></thead><tbody>{product.plans.map((plan) => <tr className="border-t border-[#d5d1c7] align-top" key={plan.id}><td className="p-4 font-black">{plan.name}</td><td className="p-4 font-black">{formatPrice(plan)}<span className="mt-1 block text-[9px] font-normal text-[#6f6b63]">{plan.price.billingNote}</span></td><td className="p-4">{plan.credits.amount ?? "官网未披露"} {plan.credits.unit}</td><td className="p-4">{plan.credits.window}<span className="mt-1 block text-[9px] text-[#6f6b63]">{plan.credits.rollover}</span></td><td className="max-w-xs p-4 leading-5">{plan.videoAllowance}</td><td className="max-w-xs p-4 leading-5">{plan.overage}</td><td className="p-4">{plan.status}</td></tr>)}</tbody></table></div> : <div className="mt-6 border border-black p-6 text-sm">没有当前 App 套餐；此条目仅保留 API/停止服务参考。</div>}</section>

    <section className="border-t border-black py-12"><div className="eyebrow">NATIVE RATES</div><h2 className="mt-2 text-3xl font-black tracking-[-.04em]">模型原生消耗</h2><p className="mt-3 max-w-2xl text-xs leading-5 text-[#6f6b63]">同名 credits 只在本产品计费体系内理解。只有官网同时公开额度价格与模型扣费时，才显示 5 秒现金折算。</p><div className="mt-7 overflow-x-auto border hairline"><table className="w-full min-w-[980px] border-collapse text-left text-xs"><thead className="bg-black text-white"><tr>{["模型", "模式", "分辨率", "时长", "音频", "原生费率", "5 秒折算", "说明"].map((x) => <th className="p-4" key={x}>{x}</th>)}</tr></thead><tbody>{product.rates.map((rate) => <tr className="border-t border-[#d5d1c7] align-top" key={rate.id}><td className="p-4 font-black">{rate.model}</td><td className="p-4">{rate.mode}</td><td className="p-4">{rate.resolution}</td><td className="p-4">{rate.durationSeconds ? `${rate.durationSeconds} 秒` : "按秒/可变"}</td><td className="p-4">{rate.audio === null ? "因模型而异" : rate.audio ? "有" : "无"}</td><td className="p-4 font-black">{formatRate(rate)}</td><td className="p-4 font-black">{formatFiveSecondCost(rate)}{rate.fiveSecondCost ? <span className="mt-1 block max-w-44 text-[9px] font-normal leading-4 text-[#6f6b63]">{rate.fiveSecondCost.basis}</span> : null}</td><td className="max-w-sm p-4 leading-5 text-[#625e57]">{rate.note}</td></tr>)}</tbody></table></div></section>

    <section className="grid gap-8 border-t border-black py-12 lg:grid-cols-[300px_1fr]"><div><div className="eyebrow">EVALUATION</div><h2 className="mt-2 text-2xl font-black">怎么判断适不适合</h2></div><div className="grid gap-px bg-[#d5d1c7] sm:grid-cols-2"><div className="bg-[var(--paper)] p-5"><div className="text-[9px] font-black text-[#6f6b63]">能力范围</div><p className="mt-3 text-xs font-bold leading-5">{product.capabilities.join(" / ")}</p></div><div className="bg-[var(--paper)] p-5"><div className="text-[9px] font-black text-[#6f6b63]">失败任务</div><p className="mt-3 text-xs font-bold leading-5">{product.failedRefund}</p></div><div className="bg-[var(--paper)] p-5 sm:col-span-2"><div className="text-[9px] font-black text-[#6f6b63]">模型范围</div><p className="mt-3 text-xs font-bold leading-5">{product.models.join(" / ")}</p></div></div></section>
