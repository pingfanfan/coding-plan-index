...
export function CompareTool({ products }: { products: Product[] }) {
  ...
  return (
    <div className="shell py-12 md:py-16">
      ...
      {selected.length > 0 ? <div className="scrollbar-none mt-8 overflow-x-auto border-t border-black"><div className="min-w-[760px]">{rows.map(([label, render]) => <div key={label} className="grid border-b hairline" style={{ gridTemplateColumns: `150px repeat(${selected.length}, minmax(0, 1fr))` }}><div className="bg-black p-4 text-xs font-black text-white">{label}</div>{selected.map((item) => <div key={item.id} className="whitespace-pre-line border-l hairline p-4 text-xs leading-5 text-[#514d47]">{render(item)}</div>)}</div>)}</div></div> : <div className="mt-10 grid min-h-64 place-items-center border hairline"><div className="text-center"><div className="mx-auto grid h-10 w-10 place-items-center bg-black text-white"><Check size={17} /></div><p className="mt-4 text-sm font-black">先添加 2–4 个套餐</p><p className="mt-2 text-xs text-[#6f6b63]">也可以从产品卡或详情页直接加入。</p><Link href="/#catalog" className="mt-5 inline-block border border-black px-4 py-2 text-xs font-black">浏览产品</Link></div></div>}
    </div>
  );
}
