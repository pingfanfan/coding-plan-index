    ...
    <footer className="mt-24 border-t border-black bg-black py-10 text-white">
      <div className="shell flex flex-col justify-between gap-8 md:flex-row">
        <div>
          ...
          <p className="mt-2 text-[10px] text-white/40">匿名访问统计由 Cloudflare Web Analytics 提供；订阅只保留聚合数字，不使用 Cookie。</p>
        </div>
        <div className="flex flex-wrap gap-x-6 gap-y-3 text-xs font-bold text-white/70">
          <Link href="/offers">活动与临时权益</Link><Link href="/free-models">免费模型雷达</Link><Link href="/analytics">订阅数据</Link><Link href="/methodology">比较方法</Link><Link href="/sources">官方来源</Link><Link href="/benchmarks">独立评测</Link>
        </div>
      </div>
    </footer>
  );
