    ...
    <section className="shell py-12 md:py-16" id="catalog">
      ...
      <div className="mt-8 grid gap-px border hairline bg-[#d5d1c7] lg:grid-cols-2">
        {filtered.map((product) => { const plan = entryPlan(product); const planKey = plan ? `${product.slug}::${plan.id}` : null; const isSelected = planKey ? selected.includes(planKey) : false; return <article key={product.slug} className="flex min-h-[320px] flex-col bg-[var(--paper)] p-5 md:p-6">
          <div className="flex items-start justify-between gap-4"><div className="flex items-center gap-3"><span className="h-4 w-4 rounded-full border border-black" style={{ background: product.accent }} /><div><div className="text-[9px] font-black text-[#817c73]">{kindName[product.kind]} · {product.status === "current" ? "当前" : "已停止"}</div><h2 className="mt-1 text-2xl font-black tracking-[-.04em]">{product.name}</h2></div></div><div className="text-right"><div className="text-xl font-black">{price(plan)}</div><div className="mt-1 text-[9px] text-[#817c73]">{plan?.name ?? "无当前套餐"}</div></div></div>
          <p className="mt-5 text-xs leading-5 text-[#625e57]">{product.summary}</p>
          <div className="mt-5 grid grid-cols-2 gap-px bg-[#d5d1c7] text-[10px]"><div className="bg-white/55 p-3"><span className="text-[#817c73]">最高分辨率</span><strong className="mt-1 block">{product.maxResolution}</strong></div><div className="bg-white/55 p-3"><span className="text-[#817c73]">原生音频</span><strong className="mt-1 block">{product.nativeAudio === null ? "因模型而异" : product.nativeAudio ? "支持" : "未支持"}</strong></div><div className="bg-white/55 p-3"><span className="text-[#817c73]">入口额度</span><strong className="mt-1 block">{plan?.credits.amount ?? "官网未披露"} {plan?.credits.unit ?? ""}</strong></div><div className="bg-white/55 p-3"><span className="text-[#817c73]">API</span><strong className="mt-1 block">{product.apiAvailable ? "有" : "未提供"}</strong></div></div>
